...
**Output style:** surface clean informational progress per [`../../references/output-style.md`](../../references/output-style.md) — capture mechanical output (the `re-detect` verdict JSON, each agent's return, `re-emit`'s write log, the agents' tool stdout), emit ONE `✓` / `→` / `✗` step line per meaningful step, and never dump raw script/JSON/`find`/`grep` output into the user's transcript. On a BLOCKER, run the §3 R2 self-heal protocol (informational error → `AskUserQuestion`: report-and-stop or self-heal-and-continue).
...
3. **`landscape-researcher` after `dependency-mapper`** — it annotates dependency-mapper's external-dependency table, so it needs that table (plus code-inventory's tech detections) as input.
   - Dispatch `reverse-engineer:landscape-researcher` (`model: opus`) with the header + the **P1 — landscape-researcher** body; thread code-inventory's **detections** + dependency-mapper's **external-dep table** into the `detections.{{...}}` slots. Pass `offline` if the session has no network (it degrades honestly — **never** falls back to training data).
